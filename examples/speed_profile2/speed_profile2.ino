...
const uint32_t BUTTON_PIN = 29;  // ボタンを接続するGPIOピン
...
C620 motor_c620(manager, C6x0Id::ID_3);
...
bool timer_int(repeating_timer_t *t) {
  ...
  if (profile_running) {
    if (tick >= xbar_ref.size()) {
      ...
      motor_c620.setCurrent(0.0f);
    } else {
      const float target_velocity = xbar_ref[tick];
      const float current_velocity = motor_c620.getVelocityRad();  // 現在の速度を取得

      // ログを記録 (最初の1000サンプルのみ)
      if (tick < LOG_SIZE) {
        logs[tick].targetVelocity = target_velocity;
        logs[tick].currentVelocity = current_velocity;
        logs[tick].currentRad = motor_c620.getAccumPositionRad() / 19.0f;
      }
      const float output_current = motor_pid.solve(target_velocity - current_velocity);

      motor_c620.setCurrent(output_current);
      tick++;
    }
  } else {
    motor_c620.setCurrent(0.0f);  // 実行中でなければモーターを停止
  }
